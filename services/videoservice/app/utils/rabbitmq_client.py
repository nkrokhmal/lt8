...
class RabbitMQClient:
    def __init__(self, callback=None):
        ...
        self.config = config
        self.channel_name = config.RABBITMQ_QUEUE_VIDEO_NAME
    ...
    def handler(self, channel, method, properties, body):
        ...
        message = thread.join()
        if message:
            self.channel.queue_declare(queue=config.RABBITMQ_QUEUE_AUDIO_NAME)
            self.publish(config.RABBITMQ_QUEUE_AUDIO_NAME, message)
        else:
            print("Exception occurred, message is None")
