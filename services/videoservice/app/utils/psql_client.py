...
class PostgresClient:
    ...
    @reconnect
    def update_conference(self, conference_id, extension="wav"):
        try:
            cur = self.client.cursor()
            audio_url = f"https://{self.config.SFTP_HOST}/audios/{conference_id}.{extension}"

            req = 'UPDATE "{}" SET  "audio_url" = \'{}\' WHERE "id" = \'{}\'' \
                .format('conferences', audio_url, conference_id)
            cur.execute(req)
            self.client.commit()
            cur.close()
        except Exception as e:
